...
########################################################################
def perc(x,listD,listS,flag):
    total_array = np.array(listD,dtype=float)
    ix = listS.index(x)
    total_x_array = total_array[ix:ix+5]
    if(flag == 1):
        listE_Total.append(list(total_x_array))
    if(flag == 2):
        listP_Total.append(list(total_x_array))
    listFinal = []
    for i in range(4):
...
#####################################################################
for i in statesList:
    total_percE = perc(i,totalE,df_statesE,1)
    total_percP = perc(i,totalP,df_statesP,2)
    dataset = []
    test = []
    train = []
    tempList = []
...
##############################################################################
def predict():
    df_predict_display = pd.DataFrame({'State':statesList,'2017A':df_17List,'2017P':Predicted_2017E,"%GrowthA":tempListP,"%GrowthP":finalpredicted})
    ...
    india.readshapefile('/home/prathamesh/SDL_Project/Shapefile/India','areas')
    ...
    perIncreaseP = list(Predicted_2017E)
    perIncreaseA = list(df_17List)
    ...
    ax.set_xticklabels(statesList,rotation='vertical')
    plt.legend()
    plt.tight_layout()
...
def applot(state):
    ...
    states = states.iloc[:,0]
    statesList = list(states)
    listStates = ['2013','2014','2015','2016','2017A','2017P']
    
    indexS = statesList.index(state)
    listTemp = []
    listTemp = listE_Total[indexS]
    #listTemp.append(Predicted_2017E[indexS])
    listP = Predicted_2017E[indexS]
    fig = plt.figure()
    width = .5
    ind = np.arange(len(listTemp))
    ind2 = 1
    rects1 = plt.bar(ind,listTemp,width,color='gold')
    rects2 = plt.bar(ind2+width*8,listP,width,color='red')
    plt.xlabel('Year')
    plt.ylabel('Energy Growth For State')
    
    plt.xticks(ind,(listStates))
